...
fn main(){
    ...
    if args.len()<3 {
        println!("Inject.exe 的使用说明是 <process> <dll path>");
        println!("尝试从ini中读取信息");
        let conf = Ini::load_from_file("cfg.ini").unwrap();

        let section = conf.section(Some("Inject")).unwrap();
        let YuanShenPath = section.get("GenshinPath").unwrap();
        let dll = "CLibrary.dll";
        inject_dll(&YuanShenPath,&dll);
        std::process::exit(1);
    }
    inject_dll(&args[1],&args[2]);
...
fn inject_dll(process_name:&str,file_path:&str){
